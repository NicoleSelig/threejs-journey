...
for(let i = 0; i < particlesCount; i++) {
    ...
    positions[i * 3 + 1] = objectsDistance * 0.4 - Math.random() * objectsDistance * 3 * sectionMeshes.length
...
const particlesMaterial = new THREE.PointsMaterial({
    color: parameters.materialColor,
    sizeAttenuation: true, // perspective particles
    size: 0.03
})
...
const tick = () =>
{
    ...
    cameraGroup.position.x += (parallaxX - cameraGroup.position.x)/10 * deltaTime * 5 // delta time handles the frame rate, same speed for different screens
    cameraGroup.position.y += (parallaxY - cameraGroup.position.y)/10 * deltaTime * 5
